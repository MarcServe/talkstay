...
const SENTIMENT_STYLE: Record<string, string> = {
  positive: "border border-emerald-200 dark:border-emerald-400/30 bg-emerald-100 dark:bg-emerald-400/15 text-emerald-800 dark:text-emerald-200",
  neutral: "border border-slate-200 dark:border-slate-400/30 bg-slate-100 dark:bg-slate-400/15 text-slate-600 dark:text-slate-300",
  negative: "border border-rose-200 dark:border-rose-400/30 bg-rose-100 dark:bg-rose-400/15 text-rose-800 dark:text-rose-200",
};
const SEVERITY_STYLE: Record<string, string> = {
  high: "border border-rose-200 dark:border-rose-400/30 bg-rose-100 dark:bg-rose-400/15 text-rose-800 dark:text-rose-200",
  medium: "border border-amber-200 dark:border-amber-400/30 bg-amber-100 dark:bg-amber-400/15 text-amber-900 dark:text-amber-200",
  low: "border border-slate-200 dark:border-slate-400/30 bg-slate-100 dark:bg-slate-400/15 text-slate-600 dark:text-slate-300",
};
...
function Stat({ icon: Icon, label, value, sub, active, onClick }: {
  ...
  return (
    ...
    >
      ...
      {clickable && (
        <div className={`mt-2 text-[10px] font-medium uppercase tracking-wide ${active ? "text-teal-700 dark:text-teal-200" : "text-muted-foreground/80"}`}>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs">
            ...
            {dayFilter && (
              <button type="button" onClick={() => setDayFilter(null)}
                className="inline-flex items-center gap-1 rounded-full border border-sky-200 dark:border-sky-400/30 bg-sky-100 dark:bg-sky-400/15 px-2.5 py-1 font-medium text-sky-800 dark:text-sky-200">
                {charts.hourly
            ...
            {deptFilter && (
              <button type="button" onClick={() => setDeptFilter(null)}
                className="inline-flex items-center gap-1 rounded-full border border-teal-200 dark:border-teal-400/30 bg-teal-100 dark:bg-teal-400/15 px-2.5 py-1 font-medium text-teal-900 dark:text-teal-200">
            ...
            {statusFilter && (
              <button type="button" onClick={() => setStatusFilter(null)}
                className="inline-flex items-center gap-1 rounded-full border border-amber-200 dark:border-amber-400/30 bg-amber-100 dark:bg-amber-400/15 px-2.5 py-1 font-medium text-amber-900 dark:text-amber-200">
            ...
            {propertyFilter && (
              <button type="button" onClick={() => setPropertyFilter(null)}
                className="inline-flex items-center gap-1 rounded-full border border-violet-200 dark:border-violet-400/30 bg-violet-100 dark:bg-violet-400/15 px-2.5 py-1 font-medium text-violet-900 dark:text-violet-200">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-x-8 gap-y-3">
            <div>
              ...
              <div className="flex items-baseline gap-2">
                ...
                {delta != null && (
                  <span className={`inline-flex items-center gap-1 text-xs font-medium ${delta < 0 ? "text-green-600 dark:text-green-300" : delta > 0 ? "text-red-600 dark:text-red-300" : "text-muted-foreground"}`}>
              ...
              {issues.slice(0, 8).map((i: any) => {
                ...
                return (
                  <div key={i.key} className="flex items-center gap-3 px-4 py-2.5 text-sm">
                    ...
                    <span className={`inline-flex w-24 shrink-0 items-center justify-end gap-1 text-xs font-medium ${diff < 0 ? "text-green-600 dark:text-green-300" : diff > 0 ? "text-red-600 dark:text-red-300" : "text-muted-foreground"}`}>
        ...
        <table className="w-full min-w-[720px] text-sm">
          ...
          <tbody>
            {rows.map((r) => (
              <tr
                key={r.id}
                className="cursor-pointer border-t align-top transition-colors hover:bg-sky-50/80 dark:hover:bg-sky-400/25"
                onClick={() => onOpen(r.id)}
              >
                <td className="px-3 py-2 font-medium">{r.room}</td>
                <td className="px-3 py-2 text-muted-foreground">{deptLabel(r.department_key)}</td>
                <td className="px-3 py-2 max-w-[220px] truncate font-medium text-teal-800 dark:text-teal-200">{r.summary}{r.is_complaint ? " ⚠️" : ""}</td>
    ...
    <div className="rounded-2xl border bg-card p-4 shadow-sm">
      ...
      {empty ? (
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {/* Stay pulse — violet */}
          <div className="overflow-hidden rounded-xl border border-violet-200/80 dark:border-violet-400/30 bg-violet-50/40 dark:bg-violet-400/15">
            <div className="flex items-center justify-between gap-2 border-b border-violet-200/70 dark:border-violet-400/30 bg-violet-100/50 dark:bg-violet-400/15 px-3 py-2">
              <div className="flex items-center gap-2">
                <span className="h-2 w-2 rounded-full bg-violet-500" aria-hidden />
                <span className="text-xs font-semibold uppercase tracking-wide text-violet-900 dark:text-violet-200">Stay pulse</span>
                <span className="text-[11px] text-violet-700/80 dark:text-violet-200">{pulses.length}</span>
              </div>
              <Button
                type="button"
                size="sm"
                variant="ghost"
                className="h-7 px-2 text-violet-800 dark:text-violet-200 hover:bg-violet-100 dark:hover:bg-violet-400/25 hover:text-violet-950 dark:hover:text-violet-100"
                onClick={onOpenPulse}
              >
                All
              </Button>
            ...
            {pulseItems.length === 0 ? (
              <p className="px-3 py-4 text-xs text-violet-800/70 dark:text-violet-200">No stay checks in this range.</p>
            ) : (
              <div className="divide-y divide-violet-200/60">
                {pulseItems.map((item) => (
                  <button
                    ...
                    type="button"
                    className="block w-full border-l-[3px] border-l-violet-500 dark:border-l-violet-400 px-3 py-2.5 text-left transition-colors hover:bg-violet-100/50 dark:hover:bg-violet-400/25"
                    onClick={() => {
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-2 text-xs">
                      ...
                      {item.rating != null && (
                        <span className="font-medium text-violet-800 dark:text-violet-200">{item.rating}★</span>
                      )}
                      <span className="text-violet-800/70 dark:text-violet-200">{formatRoomLabel(item.room)} · {fmtWhen(item.at)}</span>
                    </div>
                    <p className="mt-1 truncate text-sm font-medium text-violet-950 dark:text-violet-200">{item.title}</p>
                    {item.body && (
                      <p className="mt-0.5 line-clamp-2 text-xs text-violet-900/65 dark:text-violet-200">“{item.body}”</p>
                    )}
                  </button>
                ))}
              </div>
          ...
          <div className="overflow-hidden rounded-xl border border-amber-200/80 dark:border-amber-400/30 bg-amber-50/40 dark:bg-amber-400/15">
            <div className="flex items-center justify-between gap-2 border-b border-amber-200/70 dark:border-amber-400/30 bg-amber-100/50 dark:bg-amber-400/15 px-3 py-2">
              <div className="flex items-center gap-2">
                <span className="h-2 w-2 rounded-full bg-amber-500" aria-hidden />
                <span className="text-xs font-semibold uppercase tracking-wide text-amber-950 dark:text-amber-200">Ratings</span>
                <span className="text-[11px] text-amber-800/80 dark:text-amber-200">{reviews.length}</span>
              </div>
              <Button
                type="button"
                size="sm"
                variant="ghost"
                className="h-7 px-2 text-amber-900 dark:text-amber-200 hover:bg-amber-100 dark:hover:bg-amber-400/25 hover:text-amber-950 dark:hover:text-amber-100"
            ...
            {reviewItems.length === 0 ? (
              <p className="px-3 py-4 text-xs text-amber-900/70 dark:text-amber-200">No request ratings in this range.</p>
            ) : (
              <div className="divide-y divide-amber-200/60">
                {reviewItems.map((item) => (
                  <button
                    ...
                    type="button"
                    className="block w-full border-l-[3px] border-l-amber-500 dark:border-l-amber-400 px-3 py-2.5 text-left transition-colors hover:bg-amber-100/50 dark:hover:bg-amber-400/25"
                    onClick={() => onOpenRequest(item.requestId)}
                  >
                    <div className="flex flex-wrap items-center gap-2 text-xs">
                      <span className="font-semibold tracking-wide text-amber-600 dark:text-amber-300">
                        {"★".repeat(item.rating)}
                        <span className="text-amber-300">{"★".repeat(Math.max(0, 5 - item.rating))}</span>
                      </span>
                      <span className="text-amber-900/70 dark:text-amber-200">{formatRoomLabel(item.room)} · {fmtWhen(item.at)}</span>
                    </div>
                    <p className="mt-1 truncate text-sm font-medium text-amber-950 dark:text-amber-200">{item.title}</p>
                    {item.body && (
                      <p className="mt-0.5 line-clamp-2 text-xs text-amber-950/65 dark:text-amber-200">“{item.body}”</p>
                    )}
                  </button>
                ))}
              </div>
        ...
        {feed.slice(0, 60).map((r, i) => {
          ...
          return (
            ...
            >
              ...
              <span className={`min-w-0 flex-1 truncate ${rid ? "text-violet-700 dark:text-violet-200" : "text-muted-foreground"}`}>{r.content}</span>
